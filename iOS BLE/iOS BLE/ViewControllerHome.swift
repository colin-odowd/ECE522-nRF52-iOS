...
//  iOS BLE
//
//  Created by Colin O'Dowd on 3/10/23.
//
...
class ViewControllerHome: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate {
    
    @IBOutlet weak var ssLogo: UIImageView!
    @IBOutlet weak var ssWelcome: UILabel!
    ...
    override func viewDidLoad() {
        ...
        ssLogo.layer.add(rotationAnimation, forKey: "rotationAnimation")
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        // Delay due to instant BLE connection
        Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(triggerSegue), userInfo: nil, repeats: false)
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic,
        ...
        if let value = characteristic.value {
            delegate?.receivedData(data: value)
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "homeToBLE" {
               if let viewControllerBLE = segue.destination as? ViewControllerBLE {
                   viewControllerBLE.bluetoothManager = self
                   self.delegate = viewControllerBLE
               }
           }
    }
    
    @objc func triggerSegue() {
        // Trigger the segue programmatically using the segue identifier
